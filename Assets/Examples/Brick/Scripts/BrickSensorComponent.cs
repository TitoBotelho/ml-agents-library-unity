...
// BrickSensorComponent independente: gera 3 canais one-hot (Agente, Big, Small)
// Observação total = gridSizeX * gridSizeY * 3
public class BrickSensorComponent : SensorComponent
{
    ...
    public override ISensor[] CreateSensors() => new ISensor[]{ new BrickSensor(this) };
    ...
    class BrickSensor : ISensor
    {
        readonly BrickSensorComponent c; readonly int cells; readonly int obs; readonly ObservationSpec spec;
        public BrickSensor(BrickSensorComponent comp){ c=comp; cells = Mathf.Max(1, comp.gridSizeX) * Mathf.Max(1, comp.gridSizeY); obs = cells * 3; spec = ObservationSpec.Vector(obs); }
        public ObservationSpec GetObservationSpec() => spec;
        public string GetName() => "Brick3xOneHot";
